...
def classification_diagnostics(y, y_prob, model, X) -> dict:
    ...
    if model is not None and hasattr(model, 'feature_importances_'):
        ...
        fi = [{'feature': f'f{i}', 'importance': float(s)} for i, s in enumerate(scores)]
        ...
        fi = [{'feature': f'f{i}', 'importance': float(s / total)} for i, s in enumerate(scores)]
    ...
    return {
        ...
        'hosmer_lemeshow': {'stat': round(hl_stat, 4), 'p_value': round(hl_p, 4), 'passed': hl_p > 0.05},
