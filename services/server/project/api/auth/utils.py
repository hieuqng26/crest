    ...
    def wrapper(fn):
        ...
        def decorator(*args, **kwargs):
            ...
            if current_user_token != session_token:
                e = PermissionError(
                    "Access denied because a new login is done to the account outside the session. Please login again."
                )
                logger.exception(e)
                logger.debug(
                    f"current_user_token: {current_user_token}, session_token: {session_token}"
                )
