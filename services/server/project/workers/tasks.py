...
def _write_progress(run_id: str, progress: int, message: str):
    ...
    try:
        ...
        level = 'error' if progress < 0 else ('warn' if 'warn' in message.lower() else 'info')
...
def _cv_search(plugin_cls, base_params: dict, search_cfg: dict,
               X_train, y_train, run_id: str) -> dict:
    """
    ...
    def make_cv():
        return (StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=42)
                if can_stratify else KFold(n_splits=n_folds, shuffle=True, random_state=42))
    ...
    for idx, combo in enumerate(combos):
        ...
        try:
            ...
            for train_idx, val_idx in make_cv().split(X_train, y_train if can_stratify else None):
            ...
            cv_results.append({'params': candidate, 'mean_score': None, 'error': str(exc)})
            continue

        mean_score = float(np.mean(fold_scores))
        std_score  = float(np.std(fold_scores))
        cv_results.append({'params': candidate, 'mean_score': mean_score, 'std_score': std_score})

        if best_score is None or mean_score > best_score:
            best_score  = mean_score
            best_params = merged

        _write_progress(run_id, 35 + int(15 * (idx + 1) / total),
                        f'CV search {idx + 1}/{total} · best {scoring}={best_score:.4f}')

    return {'best_params': best_params, 'cv_results': cv_results, 'best_score': best_score}
...
def run_calibration(self, run_id: str):
    ...
    with app.app_context():
        ...
        try:
            ...
            _write_progress(run_id, 20, f'Loaded {len(df):,} rows · {len(df.columns)} columns')

            # --- 3. Feature prep ---
            feature_cols = feature_cols_json or [c for c in df.columns if c != target_col]
            ...
            if search_cfg and search_cfg.get('param_grid'):
                _write_progress(run_id, 36, f'Starting {search_cfg.get("type", "grid")} search…')
                search_result = _cv_search(plugin_cls, raw_params, search_cfg, X_train, y_train, run_id)
                raw_params    = search_result['best_params']
                cv_summary    = search_result
                _write_progress(run_id, 50, f'CV search done · best score={search_result["best_score"]:.4f}')
            ...
            # --- 7. Save artifact ---
            artifact_bytes = pickle.dumps({'model': plugin, 'scaler': scaler, 'feature_cols': feature_cols})
            artifact_path  = storage.upload_bytes(f"artifacts/{run_id}/model.pkl", artifact_bytes, 'application/octet-stream')
